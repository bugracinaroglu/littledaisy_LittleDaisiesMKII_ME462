        ...
        try:
            if command == "send_data2robot_arm":
                ...
                print(
                    ...
                    "set_mode difference | set_mode full_redraw | show_state"
                )
            else:
...
def main() -> None:
    # Clear only old runtime comparison/job JSON files. Historical Detect
    # snapshots in output/ remain untouched.
    robot_job_manager.reset_for_program_start()
    _print_access_info()

    # When enabled, send one path-free erase_all action before accepting drawing
    # jobs. The future robot controller owns the physical cleaning trajectory.
