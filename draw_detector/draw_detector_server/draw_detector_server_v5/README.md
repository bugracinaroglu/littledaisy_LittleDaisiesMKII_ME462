...
Her `python main.py` çalıştırılışında `config.py` içindeki ayarlar uygulanır:
...
- `STARTUP_OUTPUT_ERASE_ENABLED=False` iken `output/` içindeki timestamp'li
  Detect snapshot'ları korunur.
- `STARTUP_OUTPUT_ERASE_ENABLED=True` iken `output/` içindeki eski `*.json`
  snapshot dosyaları başlangıçta silinir. Klasör, `.gitkeep` ve JSON olmayan
  dosyalar korunur.
...
Output temizleme kapalıysa klasör çizim arşivi olarak kalır. Ayar açık veya
kapalı olsa da önceki çalıştırmadaki bir output dosyası otomatik aktif hedef
seçilmez; bu çalıştırmada yeni Detect yapılması gerekir.
...
- Startup output temizleme ayarının açık/kapalı davranışı
