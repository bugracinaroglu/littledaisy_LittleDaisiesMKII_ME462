...
def _normalised_points(
    ...
    *,
    ...
    for point in points:
        ...
        pixel_points.append((round(x * max_x), round(pixel_y * max_y)))
    if not pixel_points:
        return None
...
def render_strokes(
    ...
    *,
    ...
    image = np.full((panel_h, panel_w, 3), 255, dtype=np.uint8)
    for stroke in strokes:
        points = _normalised_points(
            stroke, panel_w, panel_h, bottom_left_origin=bottom_left_origin
        )
        if points is None:
            continue
        if len(points) == 1:
...
def render_robot_actions(
    actions: List[Dict[str, Any]], panel_w: int, panel_h: int
) -> np.ndarray:
    image = np.full((panel_h, panel_w, 3), 255, dtype=np.uint8)
    ...
    for action in actions:
        ...
        if action_type not in colors:
            continue
        points = _normalised_points(
            action, panel_w, panel_h, bottom_left_origin=True
        )
        if points is None:
            continue
...
def build_preview_frame(snapshot: PreviewSnapshot) -> np.ndarray:
    # Compute panel dimensions that preserve the configured aspect ratio.
    base = config.PREVIEW_PANEL_SIZE
    aw, ah = config.CANVAS_ASPECT_W, config.CANVAS_ASPECT_H
    if aw >= ah:
        panel_w = base
        panel_h = max(1, round(base * ah / aw))
    else:
        panel_h = base
        panel_w = max(1, round(base * aw / ah))

    live = render_strokes(
        snapshot.live_strokes, panel_w, panel_h, bottom_left_origin=False,
        show_points=config.PREVIEW_SHOW_SAMPLE_POINTS,
    )
    detected = render_strokes(
        snapshot.detected_strokes, panel_w, panel_h, bottom_left_origin=True,
        show_points=config.PREVIEW_SHOW_SAMPLE_POINTS,
    )
    job = render_robot_actions(snapshot.robot_actions, panel_w, panel_h)
    ...
    cv2.putText(
        footer,
        "Terminal: send_data2robot_arm | send_full_erase | set_mode difference/full_redraw | show_queue",
...
def run_native_preview() -> None:
    ...
    aw, ah = config.CANVAS_ASPECT_W, config.CANVAS_ASPECT_H
    base = config.PREVIEW_PANEL_SIZE
    _pw = base if aw >= ah else max(1, round(base * aw / ah))
    _ph = max(1, round(base * ah / aw)) if aw >= ah else base
    cv2.resizeWindow(
        WINDOW_NAME,
        _pw * 3 + 16,
        _ph + config.PREVIEW_HEADER_HEIGHT + config.PREVIEW_FOOTER_HEIGHT,
    )
    last_version = -1
