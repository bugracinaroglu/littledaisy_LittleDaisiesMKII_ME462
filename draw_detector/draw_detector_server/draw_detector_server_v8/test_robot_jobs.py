...
class DifferenceTests(unittest.TestCase):
    ...
    def test_pixel_erase_keeps_fragments_and_erases_only_gap(self):
        # The eraser removes the tail of a stroke. The surviving head is a
        # contiguous slice, so it is kept ("same") and only the removed tail is
        # erased. Matching is by geometry, not stroke id.
        old = drawing("old", [stroke("a", [
            [0.1, 0.5], [0.3, 0.5], [0.5, 0.5], [0.7, 0.5], [0.9, 0.5]
        ])])
        new = drawing("new", [
            stroke("b", [[0.1, 0.5], [0.3, 0.5], [0.5, 0.5]], parent="a"),
        ])
        actions = build_difference_actions(old, new)
        self.assertEqual([item["type"] for item in actions], ["erase", "same"])
        self.assertEqual(actions[0]["stroke_id"], "a")
        self.assertEqual(actions[0]["reason"], "partial_erase")
        # Only the removed tail points are in the erase gap.
        self.assertEqual(actions[0]["points"], [[0.7, 0.5], [0.9, 0.5]])
        self.assertEqual(actions[1]["stroke_id"], "b")
        self.assertEqual(actions[1]["reason"], "surviving_fragment")
        self.assertEqual(actions[1]["origin_stroke_id"], "a")

    def test_pixel_erase_broken_parent_chain_still_matches(self):
        # Regression: after several eraser passes the fragment's parent id no
        # longer points at any committed stroke (the real v7 bug). Geometry must
        # still recognise the fragments as survivors of stroke "a" and erase
        # only the gap -- no full redraw.
        old = drawing("old", [stroke("a", [
            [0.1, 0.5], [0.3, 0.5], [0.5, 0.5], [0.7, 0.5], [0.9, 0.5]
        ])])
        new = drawing("new", [
            stroke("frag1", [[0.1, 0.5], [0.3, 0.5]], parent="never-committed-1"),
            stroke("frag2", [[0.7, 0.5], [0.9, 0.5]], parent="never-committed-2"),
        ])
        actions = build_difference_actions(old, new)
        self.assertEqual([item["type"] for item in actions],
                         ["erase", "same", "same"])
        self.assertEqual(actions[0]["reason"], "partial_erase")
        self.assertEqual(actions[0]["points"], [[0.5, 0.5]])
        self.assertEqual(sum(1 for a in actions if a["type"] == "draw"), 0)

    def test_pixel_erase_two_fragments_single_gap(self):
        old = drawing("old", [stroke("a", [
            [0.1, 0.5], [0.3, 0.5], [0.5, 0.5], [0.7, 0.5], [0.9, 0.5]
        ])])
        new = drawing("new", [
    ...
    def test_brand_new_stroke_still_draws(self):
        # A new stroke whose parent is NOT in the committed state is genuinely
        # new and must be drawn.
        old = drawing("old", [stroke("a", [[0.1, 0.1], [0.2, 0.2]])])
        new = drawing("new", [
            stroke("a", [[0.1, 0.1], [0.2, 0.2]]),
            stroke("d", [[0.5, 0.5], [0.6, 0.6]]),
        ])
        actions = build_difference_actions(old, new)
        types = sorted(item["type"] for item in actions)
        self.assertEqual(types, ["draw", "same"])
