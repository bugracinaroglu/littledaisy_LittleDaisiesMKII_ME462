...
def handle_command(
    command_type, value, head, arms, face, face_state, control_state
):
    ...
    if command_type == "MODE":
        ...
        if requested_mode != previous_mode:
            print("Control mode: {} -> {}".format(
                previous_mode, requested_mode
            ))

        # Local touch/IMU behavior must not overwrite MANUAL or ROS commands.
