...
class Visualizer:
    ...
    def compose_status_panel(self, frame, status, panel_width=360):
        ...
        row("LCD face", status.get("face", "--"))
        row("Decision", status.get("decision", "--"))
        row("Gesture", status.get("gesture", "--"))

        section("Angles")
        row("Head pan cmd", self._safe_number(status.get("pan_angle"), 1, " deg"))
        row("Pan servo est.", self._safe_number(status.get("pan_servo_angle"), 1, " deg"))
        row("Tilt servo cmd", self._safe_number(status.get("tilt_angle"), 1, " deg"))
        ...
        cv2.putText(
            panel, "P: panel   U: tilt   C: center", (x_label, footer_y),
            font, 0.44, (150, 158, 166), 1, cv2.LINE_AA
        )
        cv2.putText(
            panel, "Q: quit", (x_label, footer_y + 22),
            font, 0.44, (150, 158, 166), 1, cv2.LINE_AA
        )
