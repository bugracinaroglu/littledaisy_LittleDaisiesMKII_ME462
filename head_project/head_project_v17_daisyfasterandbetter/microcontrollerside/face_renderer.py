...
class FaceRenderer:
    def __init__(self, lcd):
        ...
        # Temporary full-screen text overlay. A duration of 0 keeps the
        # text visible until another explicit display command clears it.
        self.text_active = False
        self.text_value = ""
        self.text_italic = False
        self.text_started_ms = 0
        self.text_duration_ms = 0
    ...
    def show_text(self, text, hold_ms=4000, italic=False):
        """Display a temporary full-screen ASCII text message."""
        clean = " ".join(str(text).replace("\r", " ").replace("\n", " ").split())
        if not clean:
            return False

        # Keep serial/LCD work bounded on the microcontroller.
        clean = clean[:80]
        try:
        ...
        self.text_value = clean
        self.text_italic = bool(italic)
        self.text_started_ms = ticks_ms()
        self.text_duration_ms = hold_ms
    ...
    def is_text_active(self):
        return bool(self.text_active)

    def _update_text_timeout(self, now):
        if not self.text_active or self.text_duration_ms <= 0:
            return
        if ticks_diff(now, self.text_started_ms) >= self.text_duration_ms:
            self.clear_text()
    ...
    def _update_auto_animation(self, now):
        ...
        if self.face in ("NEUTRAL", "CURIOUS", "HAPPY", "SAD", "IDLE", "RUNNING", "SIGMA", "THINKING"):
    ...
    def _capture_glyph(self, char, color):
        """Capture one built-in 8x8 framebuf glyph as a list of pixels."""
        tx = self.W - 8
        ty = self.H - 8
        self.lcd.fill_rect(tx, ty, 8, 8, self.BLACK)
        self.lcd.text(char, tx, ty, color)

        pixels = []
        for row in range(8):
            for col in range(8):
                if self.lcd.pixel(tx + col, ty + row) == color:
                    pixels.append((col, row))

        self.lcd.fill_rect(tx, ty, 8, 8, self.BLACK)
        return pixels

    def _draw_scaled_text_line(self, text, x, y, size, color, italic=False):
        cursor_x = int(x)
        size = max(1, int(size))

        for char in text:
            pixels = self._capture_glyph(char, color)
            for col, row in pixels:
                # A row-dependent horizontal shift approximates italic type.
                shear = int((7 - row) * size * 0.42) if italic else 0
                px = cursor_x + col * size + shear
                py = int(y) + row * size
                if px < self.W and py < self.H:
                    self.lcd.fill_rect(px, py, size, size, color)
            cursor_x += 8 * size
    ...
    def _draw_text_screen(self):
        ...
        max_lines = 5 if size == 1 else 4
        lines = lines[:max_lines]

        line_height = 8 * size
        total_height = len(lines) * line_height + max(0, len(lines) - 1) * line_gap
        start_y = max(28, int((self.H - total_height) / 2))

        # Small decorative dots keep the text mode visually related to a face.
        pulse = self.anim_counter % 3
        self._fill_circle(39, 49, 3 + pulse, self.EFFECT_BLUE)
        self._fill_circle(202, 190, 3 + (2 - pulse), self.EFFECT_BLUE)

        for index, line in enumerate(lines):
            extra_width = int(3 * size) if self.text_italic else 0
            width = len(line) * 8 * size + extra_width
            x = max(14, int((self.W - width) / 2))
            y = start_y + index * (line_height + line_gap)
            self._draw_scaled_text_line(
                line,
                x,
                y,
                size,
...
#         self._draw_label("HAPPY")
    ...
    def _draw_thinking(self, blink):
        """Thoughtful face with raised eyebrow, chin hand and thought bubbles."""
        c = self.FACE_COLOR
        blue = self.EFFECT_BLUE

        bob = (self.anim_counter // 8) % 2

        # Eyes look upward and to screen-right.
        self._draw_round_eye(
            80, 98 + bob, 21, c, blink, pupil=True, pupil_dx=5, pupil_dy=-5
        )
        self._draw_round_eye(
            151, 98 + bob, 21, c, blink, pupil=True, pupil_dx=5, pupil_dy=-5
        )

        # One eyebrow is raised more than the other.
        self._thick_line(53, 73, 101, 65, c, 2)
        self._thick_line(128, 72, 177, 80, c, 2)

        # Small uncertain mouth.
        self._thick_line(91, 159, 112, 155, c, 2)
        self._thick_line(112, 155, 132, 160, c, 2)
        self._thick_line(132, 160, 151, 156, c, 2)

        # Stylised hand resting under the chin.
        self._fill_circle(151, 179, 11, c)
        self._thick_line(143, 183, 134, 204, c, 4)
        self._thick_line(151, 185, 146, 208, c, 3)
        self._thick_line(159, 183, 159, 204, c, 2)
        self._thick_line(166, 178, 171, 198, c, 2)

        # Animated thought bubbles.
        shift = self.anim_counter % 4
        self._ellipse_outline(184, 75 - shift, 7, 6, blue, thickness=1)
        self._ellipse_outline(202, 55 - shift, 10, 8, blue, thickness=1)
        self._ellipse_outline(219, 31 - shift, 13, 10, blue, thickness=1)

        self._draw_label("THINKING")
