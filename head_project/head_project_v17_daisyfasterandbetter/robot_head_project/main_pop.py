                    ...
                    if current_thumb_up and not last_thumb_up:
                        ...
                        if nose and (left_ear or right_ear):
                            ...
                            if x2 > x1 and y2 > y1:
                                ...
                                rgb_img = cv2.cvtColor(color_resized, cv2.COLOR_BGR2RGB)
                                pil_img = Image.fromarray(rgb_img)
                                ega_palette = [
                                    0,0,0, 255,255,255, 255,0,0, 0,255,0, 0,0,255, 0,255,255, 255,0,255, 255,255,0,
                                    255,128,0, 128,0,128, 0,128,0, 0,128,128, 0,0,128, 128,0,0, 128,128,128, 192,192,192
                                ]
                                pal_img = Image.new("P", (1, 1))
                                pal_img.putpalette(ega_palette + [0] * (768 - len(ega_palette)))
                                dithered = pil_img.quantize(palette=pal_img, dither=Image.Dither.FLOYDSTEINBERG)
                                
                                raw_pixels = np.array(dithered, dtype=np.uint8)
                                packed = np.zeros((240, 120), dtype=np.uint8)
                                for x in range(0, 240, 2):
                                    packed[:, x//2] = (raw_pixels[:, x] << 4) | raw_pixels[:, x+1]
                                
                                command_sender.send_image_chunked(packed.tobytes(), image_type="GS4")
                        else:
