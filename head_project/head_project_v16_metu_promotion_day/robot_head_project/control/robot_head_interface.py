...
class RobotHeadInterface:
    ...
    Direct/manual examples after selecting MANUAL mode::
        ...
        robot_head.show_text("Hello Daisy", hold_ms=4000, italic=False)
        ...
        robot_head.celebrate(count=3, hold_ms=3000)
    ...
    def show_text(
        ...
        """Display temporary ASCII text on the RP2350 LCD."""
        allowed, source = self._source_allowed(source)
        if not allowed:
            return False
        ...
        if not clean:
            print("[Text command rejected] Empty text")
            return self._record("TEXT", source, False)

        clean = clean[:80]
        hold_ms = max(0, int(hold_ms))
        sent = self.command_sender.send_text(
            clean,
            hold_ms=hold_ms,
            italic=bool(italic),
        )
    ...
    def dance(self, count=None, hold_ms=0, source=ControlMode.MANUAL):
        """Right-centre-left-centre sway with simultaneous tilt movement."""
        return self.play_gesture("DANCE", count, hold_ms, source)

    def greet(self, nod_count=None, hold_ms=0, source=ControlMode.MANUAL):
        """Turn to each side and nod nod_count times on each side."""
        return self.play_gesture("GREET", nod_count, hold_ms, source)

    def daisy_dance(self, count=None, hold_ms=0, source=ControlMode.MANUAL):
        """DANCE head choreography plus finite two-arm rhythm."""
        return self.play_gesture("DAISY_DANCE", count, hold_ms, source)
