...
class SerialParser:
    ...
    def _parse_text(self, payload, original_command):
        """Parse TEXT commands.

        Supported forms:
          TEXT:Hello Daisy
          TEXT:4000,Hello Daisy
          TEXT:4000,1,Oopsie Daisy

        The final text field may contain commas because parsing stops after
        ...
        if len(parts) == 2:
            parsed = self._parse_nonnegative_int(parts[0].strip(), original_command)
            if isinstance(parsed, tuple):
                return parsed
            hold_ms = parsed
            text = parts[1].strip()
            ...
            italic_token = parts[1].strip().lower()
            if italic_token in ("1", "true", "yes", "italic"):
                italic = True
            elif italic_token in ("0", "false", "no", "normal"):
                italic = False
            else:
                return "ERROR", original_command
        ...
        return "TEXT", (text, hold_ms, italic)
    ...
    def parse(self, cmd):
        ...
        if upper.startswith("TEXT:"):
            return self._parse_text(cmd[len("TEXT:"):], cmd)

        if upper in ("OOPSIE_DAISY", "OOPSIE DAISY"):
            return "TEXT", ("Oopsie Daisy", 5000, True)
