...
- `greet(nod_count=2)`: two nods on the right and two on the left.
...
```python
robot_head.dance(count=2, source=ControlMode.AUTO)
robot_head.greet(nod_count=2, source=ControlMode.ROS)
```
