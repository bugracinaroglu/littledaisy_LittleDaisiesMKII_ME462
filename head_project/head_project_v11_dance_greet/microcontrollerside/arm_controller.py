...
class ArmController:
    ...
    def __init__(
        ...
        dance_amplitude_deg=25,
        dance_interval_ms=160,
        dance_beats_per_cycle=4
    ):
        ...
        self.dance_amplitude_deg = float(dance_amplitude_deg)
        self.dance_interval_ms = max(20, int(dance_interval_ms))
        self.dance_beats_per_cycle = max(2, int(dance_beats_per_cycle))
        ...
        self.dance_step = 0
        self.dance_total_steps = 0
        self.last_dance_time = ticks_ms()

        # DAISY_DANCE always uses both arms.
        right_servo_enabled = enabled
    ...
    def start_dance(self, cycles=2):
        """Start a finite two-arm alternating dance motion."""
        try:
        ...
        self.running_requested = False
        self.state = self.STATE_DANCING
        self.dance_step = 0
        self.dance_total_steps = cycles * self.dance_beats_per_cycle
        self.last_dance_time = ticks_ms()
        print("ARM DANCE START x{}".format(cycles))
    ...
    def center(self):
        ...
        if self.running_use_right_arm or self.use_right_arm_for_wave or self.enabled:
            self.right_servo.go_to_neutral()
    ...
    def _update_dance_motion(self):
        now = ticks_ms()
        if ticks_diff(now, self.last_dance_time) < self.dance_interval_ms:
            return

        self.last_dance_time = now
        sign = 1 if self.dance_step % 2 == 0 else -1

        left_target = self.left_neutral_angle + sign * self.dance_amplitude_deg
        right_target = self.right_neutral_angle - sign * self.dance_amplitude_deg
        self.left_servo.set_target_angle(left_target)
        self.right_servo.set_target_angle(right_target)

        self.dance_step += 1
        if self.dance_step >= self.dance_total_steps:
            self.state = self.STATE_LOWERING
            self.left_servo.go_to_neutral()
            self.right_servo.go_to_neutral()
    ...
    def update(self):
        ...
        if self.use_right_arm_for_wave or self.running_use_right_arm or self.enabled:
        ...
        elif self.state == self.STATE_DANCING:
            self._update_dance_motion()
