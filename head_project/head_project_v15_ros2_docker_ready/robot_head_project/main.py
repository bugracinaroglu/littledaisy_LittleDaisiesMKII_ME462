...
def create_ros_command_server(robot_head, mode_manager, command_sender):
    if not ENABLE_ROS_COMMAND_SERVER:
        return None

    try:
        from control.ros_command_server import RosCommandServer

        server = RosCommandServer(
            robot_head=robot_head,
            mode_manager=mode_manager,
            command_sender=command_sender,
            host=ROS_COMMAND_SERVER_HOST,
            port=ROS_COMMAND_SERVER_PORT,
        )
        return server if server.start() else None
    except Exception as exc:
        print("ROS Docker command server initialization error:", exc)
        return None
...
def main():
    ...
    ros_command_server = create_ros_command_server(
        robot_head,
        mode_manager,
        command_sender,
    )
    ...
    if mode_manager.is_mode(ControlMode.ROS):
        if ros2_bridge is None and ros_command_server is None:
            print("Warning: ROS mode is active but no ROS bridge is available.")
        elif ros_command_server is not None:
            print(
                "ROS Docker bridge endpoint: {}:{}".format(
                    ROS_COMMAND_SERVER_HOST,
                    ROS_COMMAND_SERVER_PORT,
                )
            )
    print_keyboard_help()
    ...
    try:
        while True:
            ...
            if key == ord("1"):
                ...
                if ros2_bridge is None and ros_command_server is None:
                    print(
                        "ROS mode selected, but neither the native ROS2 bridge "
                        "nor the Docker command server is available."
                    )
            elif key == ord("e"):
        ...
        if ros2_bridge is not None:
            ros2_bridge.close()
        if ros_command_server is not None:
            ros_command_server.close()
        if human_tracker is not None:
            human_tracker.close()
        if gesture_detector is not None:
